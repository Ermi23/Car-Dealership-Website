...
export default function SafetyFeaturesPage() {
    ...
    const fetchSafetyFeatures = async () => {
        setLoading(true);
        try {
            const response = await apiClient.get('/safty-features', { params: { page: currentPage } });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const endpoint = isEdit ? `/safty-features/${currentSafetyFeature.id}` : '/safty-features';
    ...
    const handleDeleteSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await apiClient.delete(`/safty-features/${currentSafetyFeature.id}`);
            ...
            <div className="bg-gray-50 rounded-lg shadow-lg p-2">
                <div className="w-full overflow-hidden bg-white rounded-lg shadow">
                    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center p-6 space-y-2 lg:space-y-0">
                        <h2 className="text-2xl font-bold text-gray-800">Safety Features</h2>
