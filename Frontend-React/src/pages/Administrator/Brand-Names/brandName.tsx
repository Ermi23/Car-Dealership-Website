...
export default function BrandNamesPage() {
    ...
    const fetchBrandNames = async () => {
        setLoading(true);
        try {
            const response = await apiClient.get('/brand-names', { params: { page: currentPage } });
            setBrandNames(response.data.data);
            setPaginationMeta(response.data.meta);
        } catch (error: any) {
            console.error('Error fetching brandnames:', error);
            ...
            if (error?.response?.status === 401) {
                ...
                toast.error(error?.response?.data?.message || 'Failed to fetch brand names. Please try again.');
            }
        } finally {
            setLoading(false);
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        const isEdit = currentBrandName.id !== 0;
        const endpoint = isEdit ? `/brand-names/${currentBrandName.id}` : '/brand-names';
        const method = isEdit ? 'put' : 'post';

        try {
            await apiClient[method](endpoint, {
                name: currentBrandName.name,
                description: currentBrandName.description,
            });

            // First show the success message
            toast.success(`Brand name ${isEdit ? 'updated' : 'created'} successfully!`, {
                position: "top-right",
                autoClose: 3000,
            });
            ...
            setTimeout(async () => {
                setIsOpen(false);
                setCurrentBrandName({ id: 0, name: '', description: '' });
                await fetchBrandNames();
            }, 1000);

        } catch (error: any) {
            console.error(`Error ${isEdit ? 'updating' : 'adding'} brandname:`, error);
    ...
    const handleDeleteSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await apiClient.delete(`/brand-names/${currentBrandName.id}`);
            ...
            if (error?.response?.status === 401) {
                ...
                toast.error(error?.response?.data?.message || 'Failed to delete brand name.');
            }
        }
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50 overflow-y-auto">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 m-4 relative transform transition-all">
                <h3 className="text-xl font-bold mb-4">
                    {currentBrandName.id === 0 ? 'Add BrandName' : 'Edit brandname'}
                </h3>
                ...
                <div className="relative mb-6">
                    ...
                    <div className="relative flex justify-center">
                        <span className="px-3 bg-white text-gray-500 text-sm font-medium">
                            {currentBrandName.id === 0 ? 'New Entry' : 'Update Details'}
                        </span>
                    </div>
                </div>
                    ...
                    <div className="space-y-4">
                        <div className="group">
                            ...
                            <input
                                id="name"
                                type="text"
                                value={currentBrandName.name}
                                onChange={(e) =>
                                    setCurrentBrandName({ ...currentBrandName, name: e.target.value })
                                }
                        ...
                        <div>
                            ...
                            <textarea
                                id="description"
                                value={currentBrandName.description}
                                onChange={(e) =>
                                    setCurrentBrandName({ ...currentBrandName, description: e.target.value })
                                }
    ...
    const renderDeleteModal = () => (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
            <div className="bg-white rounded shadow-lg w-full max-w-md p-6">
                <h3 className="text-xl font-bold mb-4"> Delete BrandName </h3>
    ...
    const renderEmptyState = () => (
        <div className="text-center py-12 px-4 bg-white rounded-lg">
            ...
            <h3 className="mt-4 text-lg font-medium text-gray-900">No Brand Names Found</h3>
            <p className="mt-2 text-sm text-gray-500">
                Get started by creating your first brand name.
            </p>
            <div className="mt-6">
                <button
                    onClick={() => {
            ...
            <div className="bg-gray-50 rounded-lg shadow-lg p-2">
                <div className="w-full overflow-hidden bg-white rounded-lg shadow">
                    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center p-6 space-y-2 lg:space-y-0">
                        <h2 className="text-2xl font-bold text-gray-800">Brand Names</h2>
                                ...
                                <table className="w-full border-collapse border border-gray-300">
                                    ...
                                    <tbody>
                                        {brandnames.map((brandname, index) => (
                                            <tr key={brandname.id} className="hover:bg-gray-50 transition-colors duration-150">
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                    {(currentPage - 1) * brandnamesPerPage + index + 1}
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{brandname.name}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{brandname.description}</td>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                    {new Date(brandname.created_at).toLocaleDateString('en-US', {
                                                        year: 'numeric',
                                                        month: 'short',
                                                        day: 'numeric'
                                                    })}
