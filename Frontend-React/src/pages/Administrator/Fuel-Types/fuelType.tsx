...
export default function FuelTypesPage() {
    ...
    const fetchFuelTypes = async () => {
        setLoading(true);
        try {
            const response = await apiClient.get('/feul-types', { params: { page: currentPage } });
            ...
            if (error?.response?.status === 401) {
                ...
                toast.error(error?.response?.data?.message || 'Failed to fetch FuelTypes. Please try again.');
            }
        } finally {
            setLoading(false);
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        const isEdit = currentFuelType.id !== 0;
        const endpoint = isEdit ? `/feul-types/${currentFuelType.id}` : '/feul-types';
        const method = isEdit ? 'put' : 'post';

        try {
            await apiClient[method](endpoint, {
                name: currentFuelType.name,
                description: currentFuelType.description,
            });
            ...
            setTimeout(async () => {
                setIsOpen(false);
                setCurrentFuelType({ id: 0, name: '', description: '' });
    ...
    const handleDeleteSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await apiClient.delete(`/feul-types/${currentFuelType.id}`);
            ...
            if (error?.response?.status === 401) {
                ...
                toast.error(error?.response?.data?.message || 'Failed to delete fuel type.');
            }
        }
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50 overflow-y-auto">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6 m-4 relative transform transition-all">
                <h3 className="text-xl font-bold mb-4">
                    {currentFuelType.id === 0 ? 'Add Fuel Type' : 'Edit Fuel Type'}
                </h3>
                ...
                <div className="relative mb-6">
                    ...
                    <div className="relative flex justify-center">
                        <span className="px-3 bg-white text-gray-500 text-sm font-medium">
                            {currentFuelType.id === 0 ? 'New Entry' : 'Update Details'}
                        </span>
                    </div>
                </div>
                    ...
                    <div className="space-y-4">
                        <div className="group">
                            ...
                            <input
                                ...
                                onChange={(e) =>
                                    setCurrentFuelType({ ...currentFuelType, name: e.target.value })
                                }
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                onChange={(e) =>
                                    setCurrentFuelType({ ...currentFuelType, description: e.target.value })
                                }
    ...
    const renderDeleteModal = () => (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
            <div className="bg-white rounded shadow-lg w-full max-w-md p-6">
                <h3 className="text-xl font-bold mb-4"> Delete Fuel Type </h3>
                            ...
                            <table className="w-full border-collapse border border-gray-300">
                                ...
                                <tbody>
                                    {fueltypes.map((fueltype, index) => (
                                        <tr key={fueltype.id} className="hover:bg-gray-50 transition-colors duration-150">
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                {(currentPage - 1) * fueltypesPerPage + index + 1}
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{fueltype.name}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{fueltype.description}</td>
                                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                {new Date(fueltype.created_at).toLocaleDateString('en-US', {
                                                    year: 'numeric',
                                                    month: 'short',
                                                    day: 'numeric'
                                                })}
